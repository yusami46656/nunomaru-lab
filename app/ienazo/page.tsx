...
// 最新作＝公開済み（準備中でない）作品のうち WORKS 配列の末尾に近いもの。
// 新作を昇格するたび、その作品がカバーフローの1枚目に出るようにする。
const LATEST_INDEX = WORKS.map((w, i) => (w.comingSoon ? -1 : i)).filter((i) => i >= 0).pop() ?? 0;
// 最新作を先頭に、残りは元の順序を保って続ける。
const FLOW_ORDER = [WORKS[LATEST_INDEX], ...WORKS.filter((_, i) => i !== LATEST_INDEX)];

const FLOW_ITEMS: FlowItem[] = FLOW_ORDER.map((w) => ({
